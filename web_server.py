# Pico Web Server Async Web Server, Do Not Use HTTPS To Browse To It!
# Press A to reboot back to main
...
# Bring up WiFi and connect to the router and then try the mobile hotspot
def connect_net():
    global net,net2,cset
...
# Put this here in case net_connect() gets overwritten so we grab the interface info for displaying it
wlan = network.WLAN(network.STA_IF)
net2 = wlan.ifconfig()
...
def draw_header():
    ...
    # Page Header
    LCD.fill_rect(0, 0, WIDTH, 35, color(250,194,19))
    prnt_st("Pico Web Server", 8, 8, 2, 255,255,255)
    
    y = 35 + int(LINE_HEIGHT / 2)

    if net2:
        prnt_st("> LOCAL IP: ", 5, y, 2, 0,0,0)
        y += LINE_HEIGHT
        prnt_st(net2[0], 5, y, 2, B,N,J)
        y += LINE_HEIGHT
    else:
        prnt_st("> No network!", 5, y, 2, 0,0,0)
        y += LINE_HEIGHT
        prnt_st("> Check WiFi!", 5, y, 2, 0,0,0)
        
    if client_ip and client_ip != "None":
        prnt_st("> Client IP: ", 5, y, 2, 0,0,0)
        y += LINE_HEIGHT
        prnt_st(client_ip, 5, y, 2, B, N, J)
        y += LINE_HEIGHT
    
    if EST_CONNECTION == True:
        y += LINE_HEIGHT
        prnt_st("> Client Connected!", 5, y, 2, 0,0,0)
        EST_CONNECTION = False
    else:
        prnt_st("> ", 5, y, 2, 0, 0, 0)
        prnt_st("No Clients!", 30, y, 2, 255, 102, 102)
        EST_CONNECTION = False
...
async def serve_client(reader, writer):
    ...
    while await reader.readline() != b"\r\n":
        # Add a catch for button press here at the top of the loop
        if pressed:
            # Perform a soft reset
            machine.soft_reset()
            
        if keyA.value() == 0 and not pressed:
            pressed = True
            # Perform a soft reset
            machine.soft_reset()
        pass
