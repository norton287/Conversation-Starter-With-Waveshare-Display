...
def draw_page():
    ...
    LCD.fill_rect(0, 0, WIDTH, 30,color(250,194,19))
    prnt_st("Weather", 10, 6,2,255,255,255)
    ...
    if temperature is not None:
        ...
        prnt_st("Temperature:", 10, 35, 2, K,L,M)
        prnt_st(f">> {temperature_f:.2f} F", 10, 55, 2, K,L,M)
        prnt_st(f"Wind: ({winddirection})", 10, 85, 2, K,L,M)
        prnt_st(f">> {windspeed:.2f} mph", 10, 105, 2, K,L,M)
        prnt_st(f"Conditions:", 10, 130, 2, K,L,M)
        prnt_st(f">> {current_condition}", 10, 150, 2, K,L,M)
        prnt_st(f"Last update:", 10, 175, 2, K,L,M)
        prnt_st(f"{date}", 10, 195, 2, K,L,M)
        prnt_st(f"{time}", 10, 215, 2, K, L, M)

    else:
        LCD.fill_rect(0, 60, WIDTH, 30,250,194,19)
        prnt_st("Unable to display weather! Check your network settings", 5, 65, 2, K,L,M)
